...
## Rodando o Website
Atualmente existem duas maneiras de acessar o Divisor de Sílabas, descritas abaixo.

Com o Render (remotamente):
- O site pode ser acessado em [**divisorsilabas.onrender.com ↗**](https://divisorsilabas.onrender.com)

Com o Python (no Windows):
1. Baixe os arquivos (pasta compactada) deste repositório e abra um prompt de comando na pasta do projeto;
2. No terminal, insira o comando `pip install -r requirements-dev.txt` para instalar todas as dependências necessárias.
3. Ainda no terminal, execute o comando `python manage.py runserver` e pronto! Você também pode customizar seu próprio filtro de palavras e suas "palavras secretas" nos arquivos [`rotten.txt`](./eggs-dev/rotten.txt) e [`secrets.json`](./eggs-dev/secrets.json), respectivamente.
